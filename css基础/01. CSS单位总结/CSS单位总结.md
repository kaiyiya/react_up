...
为了更容易理解 *em* 和 *rem* 的区别，我们可以用生活中的比喻来解释：

### em 就像家族遗传特征

想象一下家族的身高遗传：

- 你（某个元素）的身高会受到你父亲（父元素）的影响
- 你儿子（子元素）的身高又会受到你的影响
- 一代影响一代，形成"累积效应"
- 如果家族有"放大基因"，那后代可能会越来越大
...
.child {
  font-size: 1.5em; /* 基于父元素，变成 30px */
}

.grandchild {
  font-size: 1.5em; /* 再基于父元素，变成 45px */
}
```
...
- 无论你是谁的孩子，你的基因始终来自祖先（html根元素）
- 你的孩子也直接继承祖先的基因，不受你的影响
- 每个人的基因都直接追溯到同一个源头
...
.parent {
  font-size: 1.5rem; /* 基于根元素，变成 24px */
}

.child {
  font-size: 1.5rem; /* 同样基于根元素，还是 24px */
}

.grandchild {
  font-size: 1.5rem; /* 依然基于根元素，仍然是 24px */
}
```
...
为了更好地理解 *em* 和 *rem* 的区别，让我们通过一个实际示例来比较它们：

[查看示例文件](./em-rem-example.html)
...
- *em* 是相对于**当前元素的父元素**字体大小
- 当多个元素嵌套时，*em* 会产生累积效应（compounding effect）
- 如果父元素字体大小发生变化，所有使用 *em* 的子元素都会相应调整
...
- *rem* 是相对于**根元素（html）**的字体大小
- 不受父元素字体大小影响，始终保持一致的比例关系
- 更容易维护和控制，特别适合响应式设计
